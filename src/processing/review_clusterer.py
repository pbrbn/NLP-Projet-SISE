...
class ReviewClusterer:
    ...
    def cluster_reviews(self, reviews: list[str], n_clusters: int = 3, top_n: int = 10 ) -> pd.DataFrame:
        ...
        for i in range(n_clusters):
            ...
            clusters_avis.append({
                "Cluster": f"Cluster {i + 1}",
                "Avis": cluster_reviews
            })
        
        return pd.DataFrame(clusters_data), pd.DataFrame(clusters_avis)